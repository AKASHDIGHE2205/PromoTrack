...
import { updateItem } from "../../../services/global/itemServices";
import { emptyItemForm, uomOptions, type Item, type ItemFormData } from "./types";
...
const UpdateItem: React.FC<Props> = ({ show, setShow, item, onSuccess, isEdit }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const response = await updateItem(item.item_id, form);
      if (response.success) {
      ...
      setError(typeof err === "string" ? err : "Failed to update item.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/20 backdrop-blur-md px-4">
      ...
      >
        ...
        <div className="flex items-center justify-between mb-5 border-b border-gray-200 pb-4">
          <h2 className="text-lg font-semibold text-gray-900">{isEdit ? "Update Product" : "View Product Details"}</h2>
          <button
            onClick={handleClose}
