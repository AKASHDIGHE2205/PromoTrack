...
import { updateShop } from "../../../services/global/shopServices";
import { emptyShopForm, type Shop, type ShopFormData } from "./types";
...
const UpdateShop: React.FC<Props> = ({ show, setShow, shop, onSuccess, isEdit }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const response = await updateShop(shop.shop_id, form);
      if (response.success) {
      ...
      setError(typeof err === "string" ? err : "Failed to update shop.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/20 backdrop-blur-md px-4">
      ...
      >
        ...
        <div className="flex items-center justify-between mb-5 border-b border-gray-200 pb-4">
          <h2 className="text-lg font-semibold text-gray-900">{isEdit ? "Update Shop" : "View Shop Details"}</h2>
          <button
            onClick={handleClose}
